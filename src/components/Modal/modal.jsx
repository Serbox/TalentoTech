...
function modal({open, onclose, children}) {
  return (
    ...
    <div className={`fixed inset-0 justify-center items-center transition-colors z-10 ${open ? "visible bg-black/20" : "invisible"}`} onClick={onclose}>
      {/*modal*/}
      <div onClick={e => e.stopPropagation()} className={`mx-auto mt-10 max-w-[50%] max-h-full bg-white rounded-xl shadow p-2 transition-all ${open ? "scale-100": "scale-125 opacity-0"}`}>
